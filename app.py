...
def guest_endpoint():
    ...
    try:
        oauth_response = session.post(oauth_url, data=payload, headers=headers, timeout=(5, 15))
    except requests.exceptions.ConnectTimeout as e:
        return jsonify({
            "status": "error",
            "message": "Guest login connection timed out. Verify your network, VPN, or firewall settings and retry.",
            "details": str(e)
        }), 504
    except requests.exceptions.Timeout as e:
        return jsonify({
            "status": "error",
            "message": "Guest login request timed out. The Garena OAuth endpoint may be unreachable.",
            "details": str(e)
        }), 504
